...
def main(domain_id, clas, api_key, output_file):
    federate_datasets(domain_id, clas, api_key, output_file)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='A tool that federate ods datasets by a specific class.')
    parser.add_argument('-o', '-output', type=argparse.FileType('w'), help='csv file that will contain the federation', required=True)
